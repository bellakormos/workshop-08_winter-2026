...
[deployed html url](https://bellakormos.github.io/workshop-08_winter-2026/code/workshop-code.html)
...
# creating new object called 'drought_exp_clean' 
drought_exp_clean <- drought_exp %>% 
  # cleaning up column names 
  clean_names() %>% 
  # creating a new column called species_name, relabeling species with full name   
  ...
  # adding a new column for treatment, relabel with full name
  mutate(water_treatment = case_match(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  ))  %>% 
  # making sure that categorical ariables are factors and setting factor level
...
ggplot(
  ...
  aes(x = sla, 
      y = total_g)) + 
  # for a scatterplot
  geom_point(alpha = 0.5, # add points to plot
             color = "forestgreen") + 
  # title plot, x-axis, y-axis
  labs(
    x = "Specific Leaf Area (cm\U00B2/g)",
    y = "Total Mass (g)",
    title = "As SLA increases, total mass also decreases"
  )      
...
plot(model4) # model to check diagnistics 
```
Diagnostic plots look good. 

residuals (standing in for error) look normally distributed based on QQ plot. 

Residuals look homoscedastic (have constant variance) based on 

No outliers influencing model esitmates of coefficients.
...
(bc at top of code doc, set lotus and drought stressed first)

**What does the `(Intercept)` represent?**

The intercept is the model prediction for plant mass for Lotus scoparius in the drought stressed treatment. The model predicts that Lotus scoparius mass in the drought stressed treatment wil be 0.055 $/pm$ (plus/minus)
0.025 (SE) g. 

**What does `water_treatmentWell watered` represent?**

This ia the model predition for the difference in Lotus scoparius mass between the well watered and drought stressed treatments. The model predicts that this difference is 0.12 $/pm$ 0.017 g. 


**What does `species_namePenstemon centranthifolius` represent?**

This is the model prediction for the difference between Penstemon centranthifolius mass in the well watered treatment and Lotus scoparius mass in the well watered treatment. The model predicts that this difference in 0.05 $/pm$ 0.03 g. 


**What does `species_nameEschscholzia californica` represent?**

This is the model prediction for the difference between Eschescholia californica mass in the well watered treatment and Lotus scoparius mass in the well watered treatment. The model predicts that this difference is 0.023 $/pm$ 0.03 g. 
